...
function titleFor(mode: AuthFormMode): { eyebrow: string; title: React.ReactNode; lead: string } {
  switch (mode) {
    ...
    default: return { eyebrow: "LEGENDSTUDY ACCOUNT / LOGIN", title: <>LegendStudy Account로<br />로그인하세요.</>, lead: "LAB의 공개 소개는 로그인 없이 이용할 수 있습니다. 로그인은 향후 개인 답안과 학습 기록 연결을 위한 공통 계정 기반입니다." };
  }
}
...
export function AuthForm({ mode }: { mode: AuthFormMode }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (mode === "login") {
        ...
        setMessage("로그인했습니다. LegendStudy Account가 이 브라우저에 안전하게 연결되었습니다.");
      } else if (mode === "signup") {
        const { data, error: signUpError } = await auth.client.auth.signUp({
          email: email.trim(),
          password,
          options: { emailRedirectTo: browserRedirectTo("/login/") },
        });
        if (signUpError) throw signUpError;
        setMessage(data.session ? "가입과 로그인이 완료되었습니다." : "가입 확인 이메일을 보냈습니다. 메일의 링크를 열어 이메일 확인을 완료해 주세요.");
      } else if (mode === "forgot") {
        ...
        setMessage("비밀번호를 변경했습니다. 현재 세션을 계속 사용할 수 있습니다.");
      }
    } catch {
  ...
  const signInWithProvider = async (provider: SocialProvider) => {
    ...
    setSubmitting(true);
    try {
      const { error: oauthError } = await auth.client.auth.signInWithOAuth({ provider, options: { redirectTo: browserRedirectTo("/account/") } });
    ...
    <div className="auth-page content-wrap">
      <section className="auth-card" aria-labelledby="auth-title">
        ...
        <form className="auth-form" onSubmit={submit} noValidate>
          ...
          {needsPassword && <label><span>{mode === "reset" ? "새 비밀번호" : "비밀번호"}</span><span className="auth-password-field"><input autoComplete={mode === "signup" ? "new-password" : mode === "reset" ? "new-password" : "current-password"} disabled={!canSubmit} name="password" onChange={(event) => setPassword(event.target.value)} type={showPassword ? "text" : "password"} value={password} /><button aria-label={showPassword ? "비밀번호 숨기기" : "비밀번호 표시"} className="auth-password-toggle" onClick={() => setShowPassword((visible) => !visible)} type="button">{showPassword ? "숨기기" : "표시"}</button></span></label>}
        ...
        {mode === "login" && <div className="auth-links"><Link href="/forgot-password/">비밀번호를 잊으셨나요?</Link><Link href="/signup/">회원가입</Link></div>}
        {mode === "signup" && <div className="auth-links"><Link href="/login/">이미 계정이 있으신가요? 로그인</Link></div>}
        {mode === "forgot" && <div className="auth-links"><Link href="/login/">로그인으로 돌아가기</Link><Link href="/support/">가입한 이메일을 잊으셨나요?</Link></div>}
        {mode === "reset" && <div className="auth-links"><Link href="/forgot-password/">새 재설정 링크 요청</Link><Link href="/login/">로그인으로 돌아가기</Link></div>}

        {providers.length > 0 && <div className="auth-social"><p>연결된 로그인 방식</p>{providers.map((provider) => <button className="button button--outline" disabled={submitting} key={provider} onClick={() => void signInWithProvider(provider)} type="button">{providerLabels[provider]}로 계속하기</button>)}</div>}
        <Link className="auth-back-link" href="/">LAB 메인으로 돌아가기</Link>
      </section>
    </div>
  );
}
